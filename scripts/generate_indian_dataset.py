#!/usr/bin/env python3
"""
Enhanced Dataset Generator for EV Charging Stations - India Edition
Generates realistic station data with Indian cities and locations
"""
...
STATION_TYPES = {
    'residential': {
        'ports_range': (2, 6),
        'peak_hours': [(19, 23), (7, 10)],  # Evening and morning
        'base_usage': 0.4,
        'peak_usage': 0.9
    },
    'commercial': {
        'ports_range': (4, 10),
        'peak_hours': [(9, 13), (14, 18)],  # Business hours
        'base_usage': 0.3,
        'peak_usage': 0.8
    },
    'highway': {
        'ports_range': (6, 15),
        'peak_hours': [(10, 20)],  # Long daytime peak
        'base_usage': 0.5,
        'peak_usage': 0.95
    },
    'downtown': {
        'ports_range': (8, 20),
        'peak_hours': [(8, 11), (17, 20)],  # Rush hours
        'base_usage': 0.35,
        'peak_usage': 0.85
    }
}

# Indian cities coordinates for realistic distribution
INDIAN_LOCATIONS = [
    {'name': 'Mumbai', 'lat': 19.0760, 'lon': 72.8777, 'radius': 0.15},
    {'name': 'Delhi', 'lat': 28.6139, 'lon': 77.2090, 'radius': 0.12},
    {'name': 'Bangalore', 'lat': 12.9716, 'lon': 77.5946, 'radius': 0.10},
    {'name': 'Hyderabad', 'lat': 17.3850, 'lon': 78.4867, 'radius': 0.08},
    {'name': 'Chennai', 'lat': 13.0827, 'lon': 80.2707, 'radius': 0.08},
    {'name': 'Pune', 'lat': 18.5204, 'lon': 73.8567, 'radius': 0.08},
    {'name': 'Kolkata', 'lat': 22.5726, 'lon': 88.3639, 'radius': 0.08},
    {'name': 'Ahmedabad', 'lat': 23.0225, 'lon': 72.5714, 'radius': 0.06},
]
...
def calculate_usage_pattern(hour, station_type, day_of_week):
    ...
    if day_of_week >= 5:  # Weekend
        if station_type == 'commercial':
            return config['base_usage'] * 0.4  # Less commercial usage
        elif station_type == 'residential':
            return config['base_usage'] * 1.3  # More residential usage
        else:
...
def generate_realistic_address(station_type, location_name, station_id):
    """Generate realistic Indian addresses based on station type and location"""
    street_types = {
        'residential': ['Street', 'Road', 'Lane', 'Colony', 'Nagar'],
        'commercial': ['Road', 'Plaza', 'Complex', 'Tower', 'Mall'],
        'highway': ['Highway', 'Expressway', 'Service Road', 'Bypass'],
        'downtown': ['Road', 'Street', 'Circle', 'Cross', 'Junction']
    }
    
    street_names = [
        'MG Road', 'Park Street', 'Station Road', 'Main Road', 'College Road',
        'Hospital Road', 'Temple Road', 'Market Road', 'Bank Street', 'School Road',
        'Civil Lines', 'Cantonment', 'Gandhi Road', 'Nehru Road', 'Subhash Road',
        'Indira Circle', 'Rajiv Chowk', 'Shastri Road', 'Patel Road', 'Azad Road'
    ]
    
    area_names = {
        'Mumbai': ['Bandra', 'Andheri', 'Worli', 'Dadar', 'Juhu', 'Powai'],
        ...
        'Pune': ['Koregaon Park', 'Kalyani Nagar', 'Hinjewadi', 'Viman Nagar', 'Aundh'],
        'Kolkata': ['Park Street', 'Salt Lake', 'New Town', 'Gariahat', 'Ballygunge'],
        'Ahmedabad': ['CG Road', 'SG Highway', 'Bodakdev', 'Vastrapur', 'Prahlad Nagar']
    }
    
    street_type = random.choice(street_types[station_type])
    street_name = random.choice(street_names)
    area = random.choice(area_names.get(location_name, ['Urban Area']))
    number = random.randint(100, 9999)
    
    if station_type == 'highway':
        return f"NH-{random.randint(1, 9)} Highway, {area}, {location_name}"
    else:
        return f"{number}, {street_name}, {area}, {location_name}"

def generate_indian_dataset(num_stations=50, days=14, interval_minutes=15):
    """Generate enhanced realistic Indian dataset"""
    print(f"🇮🇳 Generating Indian EV dataset: {num_stations} stations, {days} days")
    ...
    # Distribute stations across Indian cities
    for location in INDIAN_LOCATIONS:
        # Allocate stations based on city size
        location_stations = max(1, int(num_stations * 0.12))  # ~12% per city roughly
    ...
    print(f"✨ Indian EV dataset generation complete!")
    print(f"   📍 {len(stations)} stations across {len(INDIAN_LOCATIONS)} Indian cities")
    ...
    city_counts = {}
    for station in stations:
        city = station['name'].split()[0]  # Extract city name from station name
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Generate enhanced realistic Indian EV station dataset')
